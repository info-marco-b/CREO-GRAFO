...
function render() {
    ...
    */
    ...
    nodes.forEach(n => {
        ...
        // Se è un sottografo, aggiungiamo uno stile distintivo
        if (n.type === 'subgraph') {
            rect.setAttribute("stroke", "#7b1fa2"); // Bordo viola
            rect.setAttribute("stroke-width", "2");
            rect.setAttribute("rx", "10"); // Angoli più arrotondati
        }
...
// 3. Modifica la funzione updateMermaid per supportare la sintassi Mermaid dei sottografi
function updateMermaid() {
    let m = `graph ${document.getElementById('graph-dir')?.value || 'TD'}\n`;
    nodes.forEach(n => {
        if (n.type === 'subgraph') {
            // Rappresentazione testuale del link al sottografo
            m += `  ${n.id}[["Sub: ${n.subgraphTarget}"]]\n`;
        } else {
            m += `  ${n.id}["${n.label}"]\n`;
        }
    });
...
// 2. Aggiunge il nodo di tipo sottografo allo stato
function addSubgraphNode(targetGraphName) {
    const centerX = (svg.clientWidth / 2 - viewBox.x) / scale;
    const centerY = (svg.clientHeight / 2 - viewBox.y) / scale;

    const newNode = {
        id: nextId++,
        label: `Sottografo: ${targetGraphName}`,
        subgraphTarget: targetGraphName, // Riferimento al file esterno
        type: 'subgraph',
        x: centerX - 75,
        y: centerY - 40,
        w: 180, // Leggermente più largo
        h: 90,
        bgColor: '#f3e5f5' // Un colore distintivo (viola chiaro)
    };
...
// 1. Apre la modale dei grafi esistenti per aggiungere subgraph
async function openSubgraphSelector() {
    const popup = document.getElementById('load-popup');
    const listContainer = document.getElementById('graphs-list');
    
    popup.style.display = 'flex';
    listContainer.innerHTML = '<p>Caricamento grafi disponibili...</p>';
    ...
    try {
        ...
        projects.forEach(projectName => {
            // Evita di includere il progetto corrente come sottografo di se stesso
            if (projectName === currentProjectName) return;

            const div = document.createElement('div');
            div.className = 'graph-item';
            div.innerHTML = `<i data-lucide="box"></i> <span>${projectName}</span>`;
            // Quando cliccato, chiama addSubgraphNode invece di caricare il progetto
            div.onclick = () => {
                addSubgraphNode(projectName);
                closeLoadPopup();
            };
            listContainer.appendChild(div);
        });
        ...
        listContainer.innerHTML = '<p>Errore nel caricamento dei progetti.</p>'; 
    }
}
